...
export default class DiagrammerComponent implements OnInit, OnDestroy {
  ...
  public textoGraphToCodigo = signal<string>(
    "Convierte el Diagrama en Codigo \n Presiona 'Developer' seleccona \n el lenguaje"
  );
  ...
  public ngOnInit(): void {
    ...
    const dataAntiguaSala = this.serviceDiagrammer
      .getDatSala(this.datosPage.params)
    ...
    this.dataSalaSubscription = this.serviceDiagrammer
      .escucharDataSalaTrabajo()
      .subscribe((data: any) => {
        this.bandera.set(false);
        this.graph.fromJSON(JSON.parse(data));
        this.graph.clear();
    ...
    // this.dataSalaSubscription = this.serviceDiagrammer.escucharDataSalaTrabajo().sub

    this.colaboradoresSubscription = this.serviceDiagrammer
      .colaboradoresSalaTrabajo()
      .subscribe((data: ColaboradoresResponse) => {
        console.log(data);
  ...
  ngOnDestroy(): void {
    this.serviceDiagrammer
      .asistenciaBorrar(
        this.serviceHome.usuarioAuth().email,
        this.datosPage.params
      )
      .subscribe((data: any) => {
        console.log(data);
        this.serviceDiagrammer.salirSalaTrabajo(
          this.datosPage.params,
          this.serviceHome.usuarioAuth().email
        );
      });
    this.serviceDiagrammer
      .salaVacia(this.datosPage.params)
      .subscribe((data: any) => {
        console.log(data);
      });
  ...
  public initilizePizarra(instance: DiagrammerComponent) {
    ...
    const onEventLinkPointerdblclick = (linkView: any, evt: any) => {
      ...
      this.editText(
        linkView,
        `labels/${labelIndex}/attrs/labelText/text`,
        this
      );
    };
  ...
  public initilizeStencil(instance: DiagrammerComponent) {
    const { paperHeight, paperWidth, graph, paper } = instance;
    const topY = 20;
  ...
  public ngAfterViewInit(): void {
    ...
    this.serviceDiagrammer
      .getColaboradores(this.datosPage.params)
      .subscribe((data: ColaboradoresResponse) => {
        this.colaboradoresSala.set(data);
      });
  }
  ...
  entrarSalaTrabajo() {
    this.serviceDiagrammer.entrarSalaTrabajo(
      this.datosPage.params,
      this.serviceHome.usuarioAuth().email
    );
  }
  ...
  public getGraphToCodigo(lenguaje: string) {
    this.textoGraphToCodigo.set(
      `Convirtiendo Diagrama ${lenguaje}\n Espere por favor ...`
    );
    let info: string = this.obtenerDataConvert();
    this.serviceDiagrammer
      .graphTocodigo(lenguaje, info)
      .subscribe((res: GraphToCodigo) => {
        ...
        for (let i = 0; i < lineas.length; i++) {
          if (y > 280) {
            // Si la posición y excede el alto de la página (280 mm), agrega una nueva página
            doc.addPage();
